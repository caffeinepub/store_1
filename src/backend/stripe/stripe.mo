...
module {
  ...
  func buildCheckoutSessionBody(items : [ShoppingItem], allowedCountries : [Text], successUrl : Text, cancelUrl : Text, clientReferenceId : ?Text) : Text {
    ...
    // Always collect billing address (includes name) and phone number
    params.add("billing_address_collection=required");
    params.add("phone_number_collection[enabled]=true");
    // IMPORTANT: Use empty brackets [] (not [0]) for allowed_countries.
    // Using [0] causes every country to overwrite the previous one, leaving only the last country (Zimbabwe).
    // The [] notation is the correct Stripe array syntax and must never be changed back to [0].
    for (country in allowedCountries.vals()) {
      params.add("shipping_address_collection[allowed_countries][]=" # urlEncode(country));
    };
